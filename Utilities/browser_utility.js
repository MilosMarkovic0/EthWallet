...
async function isOnlinkedIn() {
    ...
    if (currentURL.includes("linkedIn.io")) {
        return true
    }
...
function loadlinkedInInNewTab() {
    chrome.tabs.create({url: `${linkedIn_CLIENT_ENDPOINT}/account/login`});
}
...
function loadlinkedInAccountsInNewTab() {
    chrome.tabs.create({url: `${linkedIn_CLIENT_ENDPOINT}/app/linkedin-accounts`});
}
